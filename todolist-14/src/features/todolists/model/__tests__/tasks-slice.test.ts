...
test("correct task should be created at correct array", () => {
  const endState = tasksReducer(
    startState,
    createTaskTC.fulfilled({ task: startState['todolistId2'][0] }, 'requestId', { todolistId: "todolistId2", title: "juice"}),
  )
...
test("correct task should change its status", () => {
  const endState = tasksReducer(
    startState,
    updateTaskTC.fulfilled({ task: {...startState['todolistId2'][1], status: TaskStatus.New} },  //здесь мы хардкодим данные которые идут в санку а дальше в экшен будто получили их от сервера
      'requestId',
...
test("correct task should change its title", () => {
  const endState = tasksReducer(
    startState,
    updateTaskTC.fulfilled({task: {...startState['todolistId2'][1], title: 'coffee'}},
      'requestId',
      { todolistId: "todolistId2", taskId: "2", 
...
test("array should be created for new todolist", () => {
  const endState = tasksReducer(startState, createTodolistTC.fulfilled({todolist: {
    id: 'qwqwqwqwqw',
    title: 'New todolist',
    addedDate: '',
    order: 0
...
test("property with todolistId should be deleted", () => {
  const endState = tasksReducer(startState, deleteTodolistTC.fulfilled({ id: "todolistId2" }, 'requestId', 
    "todolistId2"))
